...
export function maskProfileApiKey(profile = {}) {
  const normalized = normalizeProfile(profile);

  return {
    ...normalized,
    apiKey: '',
    hasApiKey: Boolean(normalized.apiKey || normalized.hasApiKey),
  };
}

export function maskProfileApiKeys(profiles) {
  return normalizeProfiles(profiles).map((profile) => maskProfileApiKey(profile));
}

export function mergeProfilesWithExisting(incomingProfiles, existingProfiles) {
  const existingById = new Map(normalizeProfiles(existingProfiles).map((profile) => [profile.id, profile]));

  return normalizeProfiles(incomingProfiles)
    .filter((profile) => profile.id)
    .map((profile) => {
      const existing = existingById.get(profile.id);
      const apiKey = profile.apiKey || existing?.apiKey || '';
