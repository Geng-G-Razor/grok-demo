...
export async function getStoredConversations({ scope = DEFAULT_SCOPE } = {}) {
  const normalizedScope = normalizeStorageScope(scope);
  ...
  return {
    ...
    scope: normalizedScope,
    configured: true,
    conversations: getDbConversations(normalizedScope),
  };
}

export async function setStoredConversations(value, { scope = DEFAULT_SCOPE } = {}) {
  const normalizedScope = normalizeStorageScope(scope);
  const conversations = normalizeConversations(value);
  const redis = getRedisClient();

  if (redis) {
    await redis.set(getStoreKey(normalizedScope), conversations);
    return {
      storage: 'upstash-redis',
      scope: normalizedScope,
      configured: true,
  ...
  setDbConversations(normalizedScope, conversations);
