...
export async function getStoredCharacters({ accountId } = {}) {
  ...
  return {
    ...
    accountId: normalizedAccountId,
    configured: true,
    characters: getDbCharacters(normalizedAccountId),
  };
}

export async function setStoredCharacters(value, { accountId } = {}) {
  const normalizedAccountId = normalizeAccountId(accountId || DEFAULT_ACCOUNT_ID);
  const characters = normalizeCharacters(value);
  const redis = getRedisClient();

  if (redis) {
    await redis.set(getStoreKey(normalizedAccountId), characters);
    return {
      storage: 'upstash-redis',
      accountId: normalizedAccountId,
      configured: true,
  ...
  setDbCharacters(normalizedAccountId, characters);
