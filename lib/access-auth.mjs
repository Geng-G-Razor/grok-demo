...
export async function createAccessAuthToken(accessUser, now = Date.now()) {
  ...
  const issuedAt = String(now);
  const signature = await hmacHex(getAccessUserSecret(accessUser), `${accessId}.${issuedAt}`);
...
export async function verifyAccessAuthToken(token, now = Date.now()) {
  ...
  const expectedSignature = await hmacHex(getAccessUserSecret(accessUser), `${accessId}.${issuedAt}`);
